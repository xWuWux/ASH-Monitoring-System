...
DATABASE_URL = os.environ.get('ASH_DATABASE_URL', 'postgresql://ash:password@localhost/ash_logs')
...
def create_token():
    ...
    return jsonify({'token': token, 'expires_in': app.config['JWT_EXPIRY_HOURS'] * 3600})
...
def get_event(event_id):
    ...
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cursor:
            cursor.execute("SELECT * FROM command_logs WHERE event_id = %s", (event_id,))
